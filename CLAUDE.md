...
## Social-mediakanalen: crossposting, monitoring en leren van echte niche-accounts (vastgesteld 2026-09-06)
Bluesky en Mastodon zijn live crossposting-kanalen — zie `SOCIAL-CHANNELS.md` voor handles en opzet. Drie losse, elk automatiseerbare onderdelen:
- **Crossposten**: `.github/workflows/social-crosspost.yml` post automatisch elk nieuw artikel uit `feed.xml` naar Bluesky en Mastodon (met hashtags `#AITools #Solopreneur #SideHustle`), elk platform met een eigen "laatst gepost"-statusbestand.
- **Metrics monitoren**: `.github/workflows/social-metrics.yml` logt dagelijks volgers/likes/reposts/replies (publieke API's, geen login nodig) in `SOCIAL-METRICS.md`. **Let op**: een steekproef (2026-09-06) op de eerste likes liet zien dat dit soort cijfers vaak bots of volledig off-niche accounts zijn — behandel ze als ruis, niet als bewijs van een gevalideerd publiek, tenzij je de accounts erachter zelf checkt.
- **Leren van echte niche-accounts**: vierde taak van de **solo-stack-growth-research-routine** (TASK 4, toegevoegd 2026-09-06) — bestudeert dagelijks één geverifieerd echt (niet-bot), actief account in de niche op Bluesky of Mastodon voor overdraagbare groei-/engagementtechniek (postfrequentie, hashtag-gebruik, threadopbouw, bio-opzet), legt dit vast in `SOCIAL-INSPIRATION.md`, en noemt daarnaast 1-3 andere geverifieerd echte accounts die de moeite waard zijn om **handmatig** te volgen. De routine volgt/liket/repost/reageert zelf nooit vanuit de Solo Stack-accounts — dat zou precies het bot-gedrag zijn dat we net signaleerden, en automatisch volgen is sowieso principieel uitgesloten (zie Automatiseringsvoorkeur).

LinkedIn (persoonlijk profiel) staat gepland maar bewust uitgesteld — zie `SOCIAL-CHANNELS.md` voor de reden (LinkedIn Page aanmaken vereist connecties die het profiel nog niet heeft; de gebruiker wacht hiermee tot er echt publiek is).
