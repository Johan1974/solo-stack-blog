...
- [ ] JSON-LD structured data aanwezig: `Article` (headline/author/publisher/datePublished/**dateModified**/mainEntityOfPage) én `FAQPage` (met dezelfde vragen als de zichtbare FAQ-sectie)
...
- [ ] Nieuw artikel is toegevoegd aan `articles.json` (root) met `title`, `path`, `category` (één van: Guides, Reviews, Comparisons, How-To, Opinion — zie `CONTENT-CALENDAR.md`-kolom "Type"), `kicker` (exact dezelfde tekst als `<p class="kicker">` in het artikel zelf, bv. "Tool review" of "Pillar guide"), `excerpt`, `date`. Nieuwe entries altijd **onderaan** toevoegen (bepaalt de volgorde bij gelijke datum).
- [ ] **`index.html`'s "Latest articles"-lijst (`#article-list`) en "Browse by category"-sidebar (`#category-index`) zijn met de hand bijgewerkt** om het nieuwe artikel mee te nemen, in dezelfde nieuwste-eerst-volgorde als `articles.json`. Dit gebeurt **niet** meer automatisch via JavaScript (was ooit zo, maar veroorzaakte een meetbare Cumulative-Layout-Shift-regressie in PageSpeed Insights — zie `SEO-TODO.md` 2026-09-05) — de homepage rendert nu bewust statisch. Kopieer de bestaande kaart-/lijst-markup als voorbeeld voor het nieuwe artikel.
- [ ] Nieuw artikel is toegevoegd aan `FRESHNESS-LOG.md` met status "nog niet gecheckt"
- [ ] Artikel bevat onderaan de content (na de FAQ) een `share-row` (zie een bestaand artikel als voorbeeld) mét `<script src="../assets/subscribe-widget.js"></script>` en een `<div data-subscribe-widget></div>` erboven — **nooit** de newsletter-inschrijfbox zelf als losse HTML overtypen: die vorm en de submit-logica leven in dat ene gedeelde bestand (`assets/subscribe-widget.js`), zodat een toekomstige fix niet weer los in elk artikel doorgevoerd hoeft te worden (dit ging eerder mis, zie git-historie 2026-09-05)
