...
| Bluesky | Automatisering klaar, wacht op account (2026-09-06) | GitHub Action + Bluesky API direct (`.github/workflows/social-crosspost.yml`) — zelfde eigen-route-keuze als LinkedIn, i.p.v. IFTTT: geen 2-applet-limiet, geen risico dat het achter een betaalmuur verdwijnt. | Post automatisch het nieuwste artikel uit `feed.xml` zodra het verschijnt. Vereist eenmalig door de gebruiker: account aanmaken + een "App Password" genereren (Bluesky-instellingen), en de secrets `BLUESKY_HANDLE`/`BLUESKY_APP_PASSWORD` zetten (Settings → Secrets and variables → Actions). |
